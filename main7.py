...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.selected_style = """
            QGraphicsView {
                border: 2px solid #00B7EB;  /* Softer cyan-blue */
                background-color: rgba(180, 165, 20, 0.3);  
            }
        """
        self.hover_effect_style = """
            QGraphicsView {
                border: 4px solid #00008B;  /* Deep blue for selection */
                background-color: rgba(232, 236, 239, 0.4);  
            }
        """
        ...
        self.traing.setStyleSheet(self.normal_style)
        self.menubar.setStyleSheet(self.normal_style)
        self.groupBox.setStyleSheet(self.normal_style)
    ...
    def handle_analysis(self):
        ...
        for target_path in self.selected_images:
            ...
            html_path = analysis.generate_error_plot()
    ...
    #     html_path = plot_analysis.generate_error_plot()
    #     self.__show_plot_window(html_path)
...
class PlotAnalysis:
    ...
    def generate_error_plot(self):
        """生成单色误差图，蓝色底色，白色高亮误差区域"""
        gt = cv2.cvtColor(cv2.imread(self.gt_path), cv2.COLOR_BGR2RGB)
        recon = cv2.cvtColor(cv2.imread(self.recon_path), cv2.COLOR_BGR2RGB)

        # 计算像素级绝对误差
        error_map = np.abs(gt.astype(float) - recon.astype(float)).mean(axis=2)
        # 归一化误差图到 [0, 1]
        ...
        fig = px.imshow(
            error_map,
            color_continuous_scale=[(0, '#0000FF'), (1, '#FFFFFF')],  # 蓝色到白色
            labels={'color': '误差强度'},
            title=f'误差分布图<br>PSNR: {psnr_value:.2f} dB   SSIM: {ssim_value:.4f}<br>'
        )

        # 保存为临时文件
        temp_dir = tempfile.mkdtemp()
        html_path = os.path.join(temp_dir, "error_plot.html")
        fig.write_html(html_path, include_plotlyjs='cdn')
        return html_path
