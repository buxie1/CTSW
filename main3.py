    ...
    def __nearestImage(self):
        ...
        if self.radonView.items() == []:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                # 将选择的索引映射到具体的高度值
                size_mapping = {0: 180, 1: 90, 2: 60, 3: 45}
...
class PlotWindow(QMainWindow):
    def __init__(self, gt_path, recon_path):
        ...
        # 窗口居中（必须在父类初始化后调用）
        self.__center()  # <- 现在可以安全调用

        # 生成图表并加载
        plot_analysis = PlotAnalysis(gt_path, recon_path)
        html_path = plot_analysis.generate_plots()
