...
# 导入UI界面
from qt.mainWindow_v12 import Ui_MainWindow  # 主窗口的UI
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def show_image_in_pretraining(self):
        ...
        try:
            # 1. 构建路径并检查
            base_dir = f"pre-Nerp/proj{self.radonAngle}/recon_img6"
            ...
            def load_next_image():
                ...
                if self.current_image_index >= len(self.image_sequence):
                    ...
                    try:
                        ...
                        temp_path = os.path.join(self.tempDir, "final_display.png")
    ...
    def show_image_in_training(self):
        ...
        try:
            # 1. 构建路径并检查
            base_dir = f"NoPre-Nerp/proj{self.radonAngle}/recon_img6"
            if not os.path.exists(base_dir):
                QMessageBox.critical(self, "路径错误", f"目录不存在：\n{base_dir}")
                return
    ...
    def angleChanged_v1(self):
        ...
        pool = ThreadPoolExecutor(max_workers=2)
        pool.submit(self.radon_GT)
        angle_index_map = {180: 0, 60: 1, 45: 2, 30: 3}
    ...
    def __nearestImage(self):
        ...
        if self.radonView.items() == []:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                # 将选择的索引映射到具体的高度值
                size_mapping = {0: 180, 1: 60, 2: 45, 3: 30}
