    ...
    def show_image_in_pretraining(self):
        ...
        try:
            if self.selected_fname:
                match = re.search(r'img_(\d+)', self.selected_fname)
                if match:
                    img_num = match.group(1)
                    base_dir = f"pre-Nerp/proj{self.radonAngle}/recon_img{img_num}"
    ...
    def show_image_in_training(self):
        ...
        try:
            if self.selected_fname:
                ...
                if match:
                    img_num = match.group(1)
                    base_dir = f"NoPre-Nerp/proj{self.radonAngle}/recon_img{img_num}"
            if not os.path.exists(base_dir):
                QMessageBox.critical(self, "路径错误", f"目录不存在：\n{base_dir}")
                return
